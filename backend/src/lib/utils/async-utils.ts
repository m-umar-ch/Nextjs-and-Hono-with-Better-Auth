 ...
 *   HONO_LOGGER.error('External service unavailable');
 * }
 * ```
 */
 ...
 * Automatically logs helpful messages for connection errors.
 *
...
export async function tryCatch<T, E = Error>(
  ...
  try {
    ...
    if (isConnectionError(error)) {
      // Import HONO_LOGGER lazily to avoid circular dependencies
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      const { HONO_LOGGER } = require("@/lib/core/hono-logger");
      HONO_LOGGER.warn(
